...
const WEATHER_CODES = {
  ...
  45: "Hmla", 48: "Hmla", 51: "Mrholenie", 53: "Mrholenie", 55: "Mrholenie",
  56: "Mrznúce mrholenie", 57: "Mrznúce mrholenie", 61: "Dážď", 63: "Dážď",
  65: "Silný dážď", 66: "Mrznúci dážď", 67: "Mrznúci dážď",
  71: "Sneženie", 73: "Sneženie", 75: "Silné sneženie", 77: "Snehové zrná",
  80: "Dažďové prehánky", 81: "Dažďové prehánky", 82: "Silné dažďové prehánky",
  85: "Snehové prehánky", 86: "Silné snehové prehánky", 95: "Búrka",
  96: "Búrka s krupobitím", 99: "Búrka s krupobitím"
};

const WIND_DIRECTIONS = ["S", "SSV", "SV", "VSV", "V", "VJV", "JV", "JJV", "J", "JJZ", "JZ", "ZJZ", "Z", "ZSZ", "SZ", "SSZ"];
...
const _formatTime = (isoStr) => {
  ...
  return d.toLocaleTimeString("sk-SK", { hour: "numeric", minute: "2-digit", hour12: false });
};  
...
export default {
  ...
  settingsSchema: [
    {
      key: "defaultCity",
      label: "Predvolené mesto",
      type: "text",
      placeholder: "Bratislava",
      description: "Mesto, ktoré sa použije, keď sa !weather spustí bez argumentu",
    },
    {
      key: "fahrenheit",
      label: "Použiť Fahrenheit",
      type: "toggle",
      description: "Zobraziť teplotu v °F namiesto °C",
    },
  ],
  ...
  async execute(args) {
    ...
    if (!query) {
      return {
        ...
        html: `<div class="command-result">
          <p>Použitie: <code>!weather &lt;mesto&gt;</code></p>
          <p>Môžete si tiež nastaviť predvolené mesto v Nastaveniach &rarr; Pluginy.</p>
        </div>`,
      };
    }
    ...
    try {
      ...
      if (!geoData.results || geoData.results.length === 0) {
        return {
          title: "Chyba počasia",
          html: `<div class="command-result"><p>Nepodarilo sa nájsť lokalitu: <strong>${query}</strong></p></div>`,
        };
      }
      ...
      return {
        title: `Počasie — ${locationName}`,
        html: _render({
          locationName,
          mainIcon,
          temp: `${Math.round(current.temperature_2m)}${curUnits.temperature_2m || "°C"}`,
          desc: WEATHER_CODES[current.weather_code] || "Neznáme",
      ...
      return {
        title: "Chyba počasia",
        html: `<div class="command-result">
          <p>Nepodarilo sa získať počasie pre <strong>${query}</strong>.</p>
          <p>Detaily chyby: <code>${errMessage || "Neznáma chyba siete"}</code></p>
        </div>`,
      };
    }
  },
